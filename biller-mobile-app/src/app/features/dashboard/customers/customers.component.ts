...
export class CustomersComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('customersListContainer') customersListContainer!: ElementRef;
  ...
  ngAfterViewInit(): void {
    ...
    if (this.customersListContainer) {
      fromEvent(this.customersListContainer.nativeElement, 'scroll')
        .pipe(
          debounceTime(200),
          takeUntil(this.destroy$)
        )
  ...
  onScroll(): void {
    const element = this.customersListContainer.nativeElement;
